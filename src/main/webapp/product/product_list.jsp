  ...
  <div class="main_container container d-flex gap-4">
	...
	<aside class="product_filter p-3 rounded shadow-sm">
	  <form method="get" action="${ctx}/product">
	    ...
	    <!-- 적용된 필터 배지들 -->
	    <c:if test="${not empty param.category or not empty param.sidoId or not empty param.sigg_area
	                 or not empty param.minPrice or not empty param.maxPrice or param.onlyAvailable=='1'}">
	        ...
	        <div class="d-flex align-items-center flex-wrap gap-2">
	          ...
	          <c:if test="${not empty selectedCategoryName}">
	            ...
	            <span class="active-filter badge bg-primary text-white p-2 d-flex align-items-center">
	              ...
	              <a href="${removeCategoryUrl}" class="btn btn-sm btn-close btn-close-white ms-2"
	                 aria-label="카테고리 제거"></a>
	            </span>
	          </c:if>
	
	          <%-- 지역 제거 (sidoId, sigg_area 동시 제거) --%>
	          <c:if test="${not empty selectedSiggName}">
	            ...
	            <span class="active-filter badge bg-success text-white p-2 d-flex align-items-center">
	              ...
	              <a href="${removeRegionUrl}" class="btn btn-sm btn-close btn-close-white ms-2"
	                 aria-label="지역 제거"></a>
	            </span>
	          </c:if>
	
	          <%-- 가격 제거 (minPrice/maxPrice 함께 제거) --%>
	          <c:if test="${not empty param.minPrice or not empty param.maxPrice}">
	            ...
	            <span class="active-filter badge bg-warning text-dark p-2 d-flex align-items-center">
	              ...
	              <a href="${removePriceUrl}" class="btn btn-sm btn-close ms-2" aria-label="가격 제거"></a>
	            </span>
	          </c:if>
	
	          <%-- 거래 가능만 제거 (onlyAvailable 제외) --%>
	          <c:if test="${param.onlyAvailable == '1'}">
	            ...
	            <span class="active-filter badge bg-info text-dark p-2 d-flex align-items-center">
	              거래 가능만
	              <a href="${removeAvailUrl}" class="btn btn-sm btn-close ms-2"
	                 aria-label="거래 가능만 제거"></a>
	            </span>
	          </c:if>
	    ...
	    <div class="form-check mb-3">
	      <input class="form-check-input" type="checkbox" id="onlyAvailable"
	             name="onlyAvailable" value="1"
	             ${param.onlyAvailable == '1' ? 'checked' : ''}>
	    ...
	    <div class="price_filter mt-4">
	      ...
	      <div class="d-flex align-items-center gap-2 mb-2">
	        <input type="number" id="minPrice" name="minPrice" class="form-control form-control-sm text-end"
	               placeholder="0" min="0" step="1000" style="max-width: 100px;"
	        ...
	        <input type="number" id="maxPrice" name="maxPrice" class="form-control form-control-sm text-end"
	               placeholder="최대" min="0" step="1000" style="max-width: 100px;"
