...
function checkForm(e) {
  e.preventDefault();
  const form = e?.currentTarget || e?.target || document.querySelector('form[name="updateMyPage"], form[name="updateMember"]');
  ...
  try {
    ...
    const $addr3= byName(form, 'addr3');
		...
		const addressInput = form.elements.namedItem('address'); // <input name="address"> 가 폼에 있어야 함
		if (addressInput) {
		  addressInput.value = [$addr1.value.trim(), $addr2.value.trim(), $addr3.value.trim()]
		                       .filter(Boolean).join(' ');
		}
		...
		// '직접입력' 모드면 mail3 값을 mail2에 주입해서 서버가 mail2로 받도록
		const isDirect = $mail3Wrapper && $mail3Wrapper.style.display !== 'none';
		if (isDirect) {
		  if (!$mail3 || !$mail3.value.trim()) { displayError('이메일 도메인을 직접 입력하세요.'); $mail3?.focus(); return false; }
		  $mail2.value = $mail3.value.trim(); // ✅ server가 mail2로 읽음
		}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const updateForm = document.querySelector('form[name="updateMyPage"], form[name="updateMember"]');
	if (updateForm) {
