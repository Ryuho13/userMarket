...
public class UserDAO {
        ...
        try (Connection conn = DBUtil.getConnection()) {
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sqlInfo)) {
                    ...
                    if (info.getProfileImg() == null || info.getProfileImg().isBlank())
    ...
    /**
     * 마이페이지 수정 (UPSERT 포함, 프로필 이미지 갱신)
     */
    public boolean updateUserAndInfo(
            int userId,
            String name,
            String phn,
            String emNullable,
            ...
            String nickname,
            String addrDetail,
            String profileImgNullable,   // 새 파일 없으면 null/blank → 기존 유지
            Integer regionIdNullable     // null 허용
    ) throws SQLException {
        ...
        // UPDATE: 새 이미지가 있을 때만 profile_img 갱신
        StringBuilder infoUpd = new StringBuilder("""
            UPDATE `user_info`
               SET nickname    = ?,
                   addr_detail = ?,
                   region_id   = ?
        ...
        if (updateProfileImg) infoUpd.append(", profile_img = ?");
        infoUpd.append(" WHERE u_id = ?");
        String sqlInfoUpdate = infoUpd.toString();

        // 없으면 새로 INSERT (UPSERT)
        String sqlInfoInsert = """
            INSERT INTO `user_info` (u_id, nickname, region_id, addr_detail, profile_img)
            VALUES (?, ?, ?, ?, ?)
        """;
        ...
        try (Connection conn = DBUtil.getConnection()) {
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sqlUser)) {
                    ...
                    if (newPwNullable == null || newPwNullable.isBlank()) {
                ...
                // 2) user_info UPDATE
                int updated;
                try (PreparedStatement ps = conn.prepareStatement(sqlInfoUpdate)) {
                    int i = 1;
                    ...
                    if (updateProfileImg) ps.setString(i++, profileImgNullable);
                    ps.setInt(i++, userId);
                ...
                // 3) 없으면 INSERT
                if (updated == 0) {
                    try (PreparedStatement ps = conn.prepareStatement(sqlInfoInsert)) {
                        ps.setInt(1, userId);
                        ps.setString(2, nickname);
                        if (regionIdNullable == null) ps.setNull(3, Types.INTEGER);
                        else ps.setInt(3, regionIdNullable);
                        ps.setString(4, addrDetail);
                        if (profileImgNullable == null || profileImgNullable.isBlank())
                            ps.setNull(5, Types.VARCHAR);
                        else
                            ps.setString(5, profileImgNullable);
                        ps.executeUpdate();
                    }
                }
